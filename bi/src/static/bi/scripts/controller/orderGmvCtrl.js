function orderGmvCtrl($scope, $http) {
	...
	$scope.datepicker = 0;
	 ...
	 $scope.search = function() {
	 	...
	 	$scope.getOrderGMV(startIntDate, endDate, type);
	 ...
	 $scope.searchLastDay = function( count ) {
	 	...
	 	$scope.getOrderGMV(startIntDate, endDate, $scope.type);
	}
	...
	$scope.getOrderGMV = function(startDate, endDate, type) {
		var req = {
			method:"GET",
		...
		.success(function(response){
			if (response.code == undefined) {
				...
				for (var i = length - 1; i >= 0; i--) {
					total += $scope.data[i].gmv;
				};
				var avg = (total/length).toFixed(2);
				document.getElementById("gmv").innerHTML = total;
	...
	function drawLineChart(space) {
      ...
      function drawTable() {
        ...
        data.addColumn('number', 'GMV Count');
    ...
    $scope.jsonToCSV = function() {
    	var length = $scope.data.length;
  		...
  		for (var i = 0; i < length; i++) {
  			...
  			json.gmv = $scope.data[i].gmv;
  			data.push(json);
  		};
  		JSONToCSVConvertor(data, "OrderGMV", true);
     }
...
angular
    .module('modeApp')
    .controller('orderGmvCtrl', orderGmvCtrl);